            ...
            float daysElapsed, float dailyJetAdvance, float deltaLonRad)
        {
            ...
            (
                ...
                (r, c) =>
                {
                    ...
                    float latRad = lat * (float)Math.PI / 180f;

                    var devX1 = 0f;
                    devX1 += dailyJetAdvance * (float)Math.Cos(latRad);

                    float ridgeDevX = 0f;
                    ridgeDevX += ridgePatternDevs[Direction.X][r, c];
                 ...
                 (r, c) =>
                 {
                     ...
                     float ridgeDevY = 0f;
                     ridgeDevY += ridgePatternDevs[Direction.Y][r, c];
